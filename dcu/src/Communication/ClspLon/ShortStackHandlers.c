...
/* Forward declarations for callback handler functions */
...
//void OnLonGoUnconfiguredReceived();

void OnLonStatusReceived(const LonStatus* const pStatus, const LonBool success);
...
{
    OnResetOccurred(pResetNotification);
}
...
void LonMsgArrived(const LonReceiveAddress* const pAddress, const LonCorrelator correlator, 
                   ...
                   const LonByte code, const LonByte* const pData, const unsigned dataLength)
{
    OnLonMsgArrived(pAddress, correlator, priority, serviceType, authenticated, code, pData, dataLength);
}
...
{
    //OnLonDomainConfigReceived(pDomain, success);
}
...
{
    //OnLonStatusReceived(pStatus, success);
}
